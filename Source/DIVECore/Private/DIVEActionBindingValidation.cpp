	...
	FDataValidationContext& Context)
{
	...
	for (int32 BindingIndex = 0; BindingIndex < Bindings.Num(); ++BindingIndex)
	{
		...
		else
		{
			...
			{
				Context.AddWarning(FText::FromString(FString::Printf(
					TEXT("Binding '%s' has a continuous action (Rotate/Unscrew/Slide) but PrimaryActionIndex is None. Set it to that action's slot so LMB-drag in Interact starts it. A binding whose only action is continuous already uses that action as LMB primary."),
					*BindingLabel)));
			}
		}
